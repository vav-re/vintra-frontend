...
function renderDocumentLibrary(filter, searchTerm) {
    ...
    }).sort(function (a, b) { return parseDate(b.date).getTime() - parseDate(a.date).getTime(); }); // Ordena por data desc
    ...
    filteredDocs.forEach(function (doc) {
        ...
        (_a = item.querySelector('.process-doc')) === null || _a === void 0 ? void 0 : _a.addEventListener('click', function (e) {
            e.stopPropagation();
            startProcessingDocument(doc.id);
        });
        (_b = item.querySelector('.download-doc')) === null || _b === void 0 ? void 0 : _b.addEventListener('click', function (e) {
            e.stopPropagation();
...
function setActiveDocumentItem(docId) {
    document.querySelectorAll('#documentList .document-item').forEach(function (item) {
...
function renderPatientDocuments() {
    ...
    patientDocs.forEach(function (doc) {
        ...
        (_c = item.querySelector('.download-doc')) === null || _c === void 0 ? void 0 : _c.addEventListener('click', function (e) { e.stopPropagation(); downloadDocument(doc.id); });
...
function setupFocusMode() {
    document.querySelectorAll('.focus-mode-btn').forEach(function (btn) {
...
function toggleFocusMode() {
    ...
    var isFocus = body.classList.contains('focus-mode');
    console.log("Modo Foco: ".concat(isFocus ? 'Ativado' : 'Desativado'));
    document.querySelectorAll('.focus-mode-btn i').forEach(function (icon) {
    ...
    showToast('info', "Modo Foco ".concat(isFocus ? 'Ativado' : 'Desativado'), isFocus ? 'Elementos da interface foram ocultos.' : 'Interface restaurada.');
}
// --- Notificações Toast ---
