...
declare global {
    interface Window {
        switchView: (viewId: string, force?: boolean) => void;
        openPatientPanel: (patientId: string) => void; // Adicionado para ser acessível pelo HTML
...
function setupEventListeners(): void {
    ...
    document.getElementById('processWithAIButton')?.addEventListener('click', processDocumentWithAI);
}
...
/** Renderiza a seleção de pacientes no dashboard */
function renderPatientSelectionOnDashboard(): void {
    ...
    container.querySelector('.patient-selection-section')?.remove();
    container.querySelector('.welcome-message')?.remove();
    container.querySelector('#recentDocuments')?.remove();
    ...
    const searchInput = document.getElementById('dashboardPatientSearch') as HTMLInputElement | null;
    if (!searchInput) {
...
function renderDocumentLibrary(filter: string = 'all', searchTerm: string = ''): void {
    ...
    if (!state.currentPatientId) {
        ...
        document.querySelector('.sidebar-link[data-target="library"]')?.classList.add('disabled');
        return;
    } else {
         document.querySelector('.sidebar-link[data-target="library"]')?.classList.remove('disabled');
    }
    ...
    filteredDocs.forEach(doc => {
        ...
        item.querySelector('.view-doc-modal')?.addEventListener('click', (e) => {
            e.stopPropagation();
            viewDocument(doc.id);
        });
        item.querySelector('.edit-doc')?.addEventListener('click', (e) => {
            e.stopPropagation();
            editDocument(doc.id);
        });
...
function setActiveDocumentItem(docId: string): void {
    ...
    document.querySelectorAll<HTMLDivElement>('#documentList .document-item, #patientDocuments .document-item').forEach(item => {
        item.classList.remove('active');
    });
    // Adiciona 'active' ao item correto em ambas as listas possíveis
    document.querySelectorAll<HTMLDivElement>(`#documentList .document-item[data-id="${docId}"], #patientDocuments .document-item[data-id="${docId}"]`).forEach(item => {
...
function viewDocumentInWorkspace(docId: string): void {
    ...
    viewContainer.querySelector<HTMLButtonElement>('.view-doc-modal')?.addEventListener('click', () => viewDocument(docId));
...
function showEmptyDocumentView(): void {
    ...
    let message = '<p class="document-empty-text">Selecione um documento da lista à esquerda para visualizá-lo aqui.</p>';
    if (!state.currentPatientId) {
...
function renderPatientDocuments(): void {
    ...
    patientDocs.forEach(doc => {
        ...
        item.querySelector<HTMLButtonElement>('.process-doc')?.addEventListener('click', (e) => { e.stopPropagation(); startProcessingDocument(doc.id); });
 ...
 */
window.openPatientPanel = function(patientId: string): void {
...
function processDocumentWithAI(): void {
    if (!state.currentDocumentId) {
...
function fixLayoutIssues() {
    ...
    if (appSidebar && appMain) {
        ...
        const updateSidebarHeight = () => {
            ...
            appMain.style.height = `calc(100vh - ${headerHeight}px)`;
        };
...
function validateDOMElements() {
    ...
    criticalElements.forEach(el => {
        ...
        if (!element) {
            console.warn(`⚠️ Elemento crítico não encontrado: ${el.name} (id: ${el.id})`);
        } else {
            console.log(`✅ Elemento encontrado: ${el.name}`);
        }
    });
}
